	...
	$.ajax({
		...
		success : function (userInfo, response, status) {
			...
			if (userInfo) { 
				...
				$("#userInfo_personHeight_modify").val(userInfo.personHeight);
				$("#userInfo_personHeight_modify").validatebox({
					required : true,
					missingMessage : "请输入身高",
				});
				$("#userInfo_personWeight_modify").val(userInfo.personWeight);
				$("#userInfo_personWeight_modify").validatebox({
					required : true,
